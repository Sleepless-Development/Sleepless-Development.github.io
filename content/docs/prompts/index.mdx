...
- FiveM / ox_lib keybind icons follow remaps under Settings > Key Bindings > FiveM
...
- **GTA controls** (`control`): keyboard and gamepad glyphs come from `GetControlInstructionalButton`. Remap INPUT_PICKUP from E to G, or move it to another pad button, and the open prompt follows.
- **FiveM / ox_lib keybinds** (`keybind`): the keyboard icon comes from the live `RegisterKeyMapping` bind. Remap it under Settings > Key Bindings > FiveM and the open prompt follows.
