...
| `keybind` | `string \| table?` | ox_lib keybind name, or the table from `lib.addKeybind` |
...
<Callout type="info">
  `keybind` sets the keyboard icon from the live ox_lib mapping. Combine it with `gamepad` so pad users still see a face or trigger icon.
</Callout>
