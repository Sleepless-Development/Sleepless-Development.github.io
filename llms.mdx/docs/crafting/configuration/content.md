# Configuration (/docs/crafting/configuration)
...
## SQL (`config/queries.lua`) [#sql-configquerieslua]

All database SQL (schema, insert, delete, save metadata, fetch) lives in `config/queries.lua` and is **not escrowed**. The schema string is filled with the active framework's character table/column types at runtime — see comments in that file.
...
### `config.inventory` [#configinventory]
...
### `config.interact` [#configinteract]
...
| `'qb-target'`          | qb-target              |
| `'sleepless_interact'` | sleepless\_interact    |
| `'drawtext'`           | Simple drawtext prompt |

### `config.UIColor` [#configuicolor]
...
### `config.blockCraftStationStorageTransfer` [#configblockcraftstationstoragetransfer]
...
### `config.versionCheck` [#configversioncheck]
