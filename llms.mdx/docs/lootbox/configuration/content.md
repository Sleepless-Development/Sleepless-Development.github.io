# Configuration (/docs/lootbox/configuration)
...
## Rarity Configuration [#rarity-configuration]
...
### Rarity Properties [#rarity-properties]
...
## Lootbox Definitions [#lootbox-definitions]
...
### Lootbox Properties [#lootbox-properties]
...
| `amount`     | `number` or `{min, max}` | Yes      | Quantity of item to give (can be a range for random values)           |
...
### Per-Lootbox Rarity Thresholds [#per-lootbox-rarity-thresholds]
...
### Item with Metadata [#item-with-metadata]
...
| `amount`   | `number` or `{min, max}` | Yes      | Quantity to give (can be a range for random values) |
...
### Random Amount Ranges [#random-amount-ranges]

The `amount` field supports both fixed values and random ranges. To specify a range, use a table with two numbers: `{min, max}`.

```lua
-- Fixed amount: always gives exactly 5
{ 40, { name = 'bread', amount = 5 } }

-- Random range: gives between 3-7 (inclusive)
{ 40, { name = 'bread', amount = { 3, 7 } } }

-- Works great for money rewards
{ 8, { name = 'money', amount = { 10000, 25000 } } }

-- Also works for bonus items
{ 40, {
    name = 'WEAPON_PISTOL',
    amount = 1,
    bonusItems = {
        { name = 'ammo-9', amount = { 40, 60 } }, -- Random ammo between 40-60
    }
} }
```

<Callout type="info">
  When using random ranges, the preview UI will display the minimum value, but players will receive a random amount within the range when they actually open the lootbox.
</Callout>

### Custom Reward Types [#custom-reward-types]
...
#### How Custom Rewards Work [#how-custom-rewards-work]
...
#### Registering Reward Hooks [#registering-reward-hooks]
...
See the [Server Exports](/docs/lootbox/exports/Server) page for full API documentation.

### Complete Vehicle Crate Example [#complete-vehicle-crate-example]
...
## Understanding Weight Distribution [#understanding-weight-distribution]
...
### Default Rarity Distribution [#default-rarity-distribution]
