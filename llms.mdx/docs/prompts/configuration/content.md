# Configuration (/docs/prompts/configuration)
...
`/promptmode` opens an ox\_lib select dialog. The player's pick is stored in KVP and overrides `config.mode` on the next start.
...
ox\_lib locales live in `locales/`. English ships as `locales/en.json`. Copy that file to add another language (`fr.json`, `de.json`, and so on). The `hold` string is prefixed on any prompt with `holdTime`.
