...
- `data` (`VehicleInteractionData`): A table containing the interaction data. It must include:
  ...
  - `offset?` (`vec3`): The offset from the vehicle's position where the interaction is located.
  ...
  - `disableWhenDead?` (`boolean`): If this is set to true the interaction will be removed when the entity is dead.
    ...
    options = {
        {
            ...
            groups = {['police'] = 1},
            items = {['money'] = 100},
            onSelect =function(data) print("Vehicle interaction triggered") end,
...
```lua
    exports.sleepless_interact:addGlobalVehicle(data)
```
